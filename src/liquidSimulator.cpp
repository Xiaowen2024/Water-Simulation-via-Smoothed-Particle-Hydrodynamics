...
void LiquidSimulator::load_textures() {
  ...
  m_gl_texture_2_size = load_texture(2, m_gl_texture_2, (m_project_root + "/textures/texture_5.png").c_str());
...
void LiquidSimulator::load_shaders() {
...
LiquidSimulator::LiquidSimulator(std::string project_root, Screen *screen)
...
void LiquidSimulator::loadLiquid(Liquid *liquid) { this->liquid = liquid; }

void LiquidSimulator::loadLiquidParameters(LiquidParameters *lp) { this->lp = lp; }

void LiquidSimulator::loadCollisionObjects(vector<CollisionObject *> *objects) { this->collision_objects = objects; }

/**
 * Initializes the cloth simulation and spawns a new thread to separate
 * rendering from simulation.
 */
void LiquidSimulator::init() {
  ...
  if (liquid) {
    avg_pm_position = Vector3D(((double)liquid->BOX_X) / 2,
                               ((double)liquid->BOX_Y) / 2,
                               ((double)liquid->BOX_Z) / 2);
    canonical_view_distance = max(liquid->BOX_X, liquid->BOX_Z) * 0.9;
  } else {
    canonical_view_distance = 10.0;
  }
...
bool LiquidSimulator::isAlive() { return is_alive; }

void LiquidSimulator::drawContents() {
  glEnable(GL_DEPTH_TEST);
  ...
  if (!is_paused) {
    ...
    for (int i = 0; i < simulation_steps; i++) {
      if (liquid && lp && collision_objects) {
        liquid->simulate(frames_per_sec, simulation_steps, *lp, external_accelerations, *collision_objects);
      }
  ...
  case PHONG:
    ...
    if (liquid) {
      shader.setUniform("u_light_pos", Vector3f(liquid->BOX_X / 2, liquid->BOX_Y * 2, liquid->BOX_Z/2), false);
    } else {
      shader.setUniform("u_light_pos", Vector3f(0.0f, 5.0f, 0.0f), false);
    }
    shader.setUniform("u_light_intensity", Vector3f(100, 100, 100), false);
  ...
  if (liquid) {
    for (Particle &p: liquid->particles) {
      p.render(shader);
    }
    shader.bind();
  }
...
void LiquidSimulator::drawPhong(GLShader &shader) {
  // No cloth mesh to draw in liquid-only mode.
  (void) shader;
}
...
void LiquidSimulator::resetCamera() { camera.copy_placement(canonicalCamera); }

Matrix4f LiquidSimulator::getProjectionMatrix() {
...
Matrix4f LiquidSimulator::getViewMatrix() {
  Matrix4f lookAt;
  Matrix3f R;
...
bool LiquidSimulator::cursorPosCallbackEvent(double x, double y) {
...
bool LiquidSimulator::mouseButtonCallbackEvent(int button, int action,
                                              int modifiers) {
  switch (action) {
  case GLFW_PRESS:
    switch (button) {
...
void LiquidSimulator::mouseMoved(double x, double y) { y = screen_h - y; }

void LiquidSimulator::mouseLeftDragged(double x, double y) {
  float dx = x - mouse_x;
  float dy = y - mouse_y;
...
void LiquidSimulator::mouseRightDragged(double x, double y) {
  camera.move_by(mouse_x - x, y - mouse_y, canonical_view_distance);
}

bool LiquidSimulator::keyCallbackEvent(int key, int scancode, int action,
                                      int mods) {
...
bool LiquidSimulator::dropCallbackEvent(int count, const char **filenames) {
  return true;
}

bool LiquidSimulator::scrollCallbackEvent(double x, double y) {
  camera.move_forward(y * scroll_rate);
  return true;
}

bool LiquidSimulator::resizeCallbackEvent(int width, int height) {
  screen_w = width;
  screen_h = height;
...
void LiquidSimulator::initGUI(Screen *screen) {
  Window *window;
