...
void ClothSimulator::loadLiquid(Liquid *liquid) { this->liquid = liquid; }

void ClothSimulator::loadLiquidParameters(LiquidParameters *lp) { this->lp = lp; }
...
void ClothSimulator::init() {
  ...
  for (auto &pm : cloth->point_masses) {
    ...
    std::cout << avg_pm_position << std::flush;
    avg_pm_position = Vector3D(0.0);
    for (Particle &p : liquid->particles) {
      avg_pm_position += p.position / ((double) (liquid->grid_len * liquid->grid_len * liquid->grid_len));
    }*/
    
    avg_pm_position = Vector3D(((double)liquid->BOX_X) / 2, ((double)liquid->BOX_Y) / 2, ((double)liquid->BOX_Z) / 2);
