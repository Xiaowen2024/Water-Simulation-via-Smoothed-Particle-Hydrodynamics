...
const unordered_set<string> VALID_KEYS = {SPHERE, PLANE, CLOTH, LIQUID};
...
bool loadObjectsFromFile(string filename, Cloth *cloth, ClothParameters *cp, Liquid *liquid, LiquidParameters *lp, vector<CollisionObject *>* objects, int sphere_num_lat, int sphere_num_lon) {
  // Read JSON from file
  ifstream i(filename);
  if (!i.good()) {
    return false;
...
int main(int argc, char **argv) {
  ...
  bool success = loadObjectsFromFile(file_to_load_from, &cloth, &cp, &liquid, &lp, &objects, sphere_num_lat, sphere_num_lon);
  if (!success) {
